...
if not CREDENTIALS_FILE.exists():
    CREDENTIALS_FILE = Path(__file__).resolve().parent.parent / "credentials" / "data" / "credentials.json"
...
class CredentialsManager(BaseManager):
    ...
    def cmd_add(self, args):
        ...
        for i, c in enumerate(existing):
            if c.get('id') == cred_id:
                if self.prompt_yesno(f"Credential '{cred_id}' exists. Overwrite?"):
                    existing[i] = new_cred
  ...
  ''' + skills_html + '''        </div>
    ...
    def cmd_generate(self, args):
        ...
        output_file = Path(__file__).resolve().parent.parent / "credentials" / "index.md"
